...
# debu
# ggl_get_spending(advertiser_id = "AR09673365123485401089", cntry = "AU",
#       start_date = as.Date(election_dat30$ds[1]) - 30,
#       end_date = election_dat30$ds[1])

days30ggl <- all_dat %>%
  drop_na(party) %>%
  # filter(page_id == "AR09673365123485401089") %>% 
  pull(page_id) %>% 
  # .[10] %>% 
  map_dfr(~{
    Sys.sleep(1)
    fin <- ggl_get_spending(
      advertiser_id = .x,
      start_date = as.Date(election_dat30$ds[1]) - 30,
      end_date = election_dat30$ds[1],
      cntry = sets$cntry
...
# the_table30 <- get_table_dat(days30ggl %>% mutate(internal_id = advertiser_id, total_spend_formatted= spend_AUD, page_name = advertiser_name ), party) %>%
#   arrange(desc(parse_number(`Total Spend`)))

the_table30 <- days30ggl %>% 
  as_tibble() %>% 
  mutate(number_of_ads = as.numeric(number_of_ads)) %>% 
  mutate(spend_AUD = as.numeric(spend_AUD)) %>% 
  group_by(party) %>% 
  summarise(`Number of Advertisers` = n(),
            spend_AUD = sum(spend_AUD, na.rm = T),
            number_of_ads = sum(number_of_ads, na.rm = T)) %>% 
  ...
  drop_na(party_col) %>% 
  arrange(desc(spend_AUD)) %>%
  mutate(`Total Spend` = scales::comma(spend_AUD)) %>%
  mutate(`Number of Ads` = scales::comma(number_of_ads)) %>% 
  select(-spend_AUD, -number_of_ads)
...
sum30 <- days30ggl %>% 
  ...
  group_by(party) %>% 
  summarise(`Number of Advertisers` = n(),
            spend_AUD = sum(spend_AUD),
            number_of_ads = sum(number_of_ads)) %>% 
  mutate(party_col = party)  %>%
    # # filter(main_currency == the_currency) %>%
    # distinct(internal_id, .keep_all = T) %>%
    summarize(total_spend_formatted = sum(spend_AUD, na.rm = T),
...
days7ggl <- all_dat %>%
  ...
  map_dfr(~{
    ...
    fin <- ggl_get_spending(
      advertiser_id = .x,
      start_date = as.Date(election_dat7$ds[1]) - 7,
      end_date = election_dat7$ds[1],
      cntry = sets$cntry
    ) %>% mutate_all(as.character) %>% 
      mutate(page_id = .x)
    # print(fin)
...
# the_table7 <- get_table_dat(days7ggl %>% mutate(internal_id = advertiser_id, total_spend_formatted= spend_AUD, page_name = advertiser_name ), party) %>%
#   arrange(desc(parse_number(`Total Spend`)))
